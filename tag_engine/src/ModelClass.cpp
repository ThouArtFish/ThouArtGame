...
void TAGModel::drawAll(const TAGShaderManager::Shader& shader, const bool& cull_face, const std::string& mesh_name) {
	...
	if (instance_buffer.was_updated) {
		...
		if ((mesh_name == "" ? meshes.at(mesh_draw_order.back()).material_mod : meshes.at(mesh_name).material_mod).opacity < 1.0f) {
...
void TAGModel::addMesh(const std::string& mesh_name, const std::vector<TAGMesh::Vertex>& vertices, const std::vector<TAGMesh::Fragment>& frags, const std::vector<TAGMesh::Texture>& textures, const TAGMesh::MaterialMod& material_mod) {
	meshes.try_emplace(mesh_name, vertices, frags, textures, material_mod);
	instances.try_emplace(mesh_name);
	instance_buffers.try_emplace(mesh_name);

	if (material_mod.opacity < 1.0f) {
...
void TAGModel::loadModel(const std::string& path) {
	...
	for (const tinyobj::shape_t& shape : shapes) {
		...
		const std::vector<tinyobj::index_t>& frag_indices = shape.mesh.indices;
		std::vector<TAGMesh::Vertex>& mesh_vertices = mesh.changeVertices();
		mesh_vertices.reserve(frag_indices.size());
		std::vector<TAGMesh::Fragment>& mesh_fragments = mesh.changeFragments();
		mesh_fragments.reserve(shape.mesh.num_face_vertices.size());
		std::vector<tinyobj::index_t> unique_indices;
		std::array<unsigned int, 3> primitive;
		size_t primitive_index = 0;
		for (size_t i = 0; i < frag_indices.size(); i++) {
			current_index = frag_indices[i];
			auto pos = std::find_if(unique_indices.begin(), unique_indices.end(), checkSameFrag);
			if (pos == unique_indices.end()) {
				primitive[primitive_index] = (unsigned int)mesh_vertices.size();
				mesh_vertices.emplace_back(all_vertices[current_index.vertex_index], all_normals[glm::abs(current_index.normal_index)], all_texcoords[glm::abs(current_index.texcoord_index)]);
				unique_indices.push_back(current_index);
			}
			else {
				primitive[primitive_index] = (unsigned int)std::distance(unique_indices.begin(), pos);
			}

			if (primitive_index == 2) {
				mesh_fragments.emplace_back(primitive);
				primitive_index = 0;
			}
			else {
				primitive_index++;
		...
		const tinyobj::material_t& material = materials[shape.mesh.material_ids.at(0)];

		if (!material.diffuse_texname.empty()) {
			mesh.textures.push_back(loadMaterialTexture(material.diffuse_texname, TAGTexType::DIFFUSE_MAP));
		}
		else {
			mesh.textures.push_back(loadMaterialTexture(*(glm::vec3*)material.diffuse, TAGTexType::DIFFUSE_MAP));
		}

		if (!material.specular_texname.empty()) {
			mesh.textures.push_back(loadMaterialTexture(material.specular_texname, TAGTexType::SPEC_MAP));
		}

		mesh.material_mod.spec_exp = material.shininess;
		mesh.material_mod.spec_mod = material.specular[0];
		mesh.material_mod.opacity = material.dissolve;
		...
		if (mesh.material_mod.opacity < 1.0f) {
